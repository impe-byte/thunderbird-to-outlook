...
// ─── HUB SCREEN ─────────────────────────────────────────────────────────────
function HubScreen({ onNavigate, t }) {
  return (
    <div className="hub-screen">
      <div className="hub-hero">
        <div className="hub-hero-eyebrow">iB_ Conversion Suite</div>
        <h1>
          Scegli la tua <span className="hub-brand-accent">conversione</span>
        </h1>
        <p className="hub-hero-desc">
          Strumenti professionali per migrare i tuoi dati da Thunderbird a Microsoft Outlook.
          100% client-side — i tuoi dati non lasciano mai il browser.
        </p>
      </div>
      ...
      <div className="hub-cards">
        ...
        >
          ...
          <div className="hub-card-cta">Inizia la conversione →</div>
        </button>

        {/* Task Card */}
        <button
          id="hub-card-task"
          className="hub-card hub-card--task"
          onClick={() => onNavigate('task-converter')}
        >
          <span className="hub-card-badge">ICS → CSV</span>
          <div className="hub-card-icon">✅</div>
          <div className="hub-card-title">Converti Attività</div>
          <div className="hub-card-desc">
            Importa le attività (VTODO) dal formato ICS di Thunderbird e convertile nel CSV Attività di Outlook.
            Mantiene priorità, scadenze e note.
          </div>
          <div className="hub-card-cta">Inizia la conversione →</div>
        </button>
      </div>
      ...
      <div className="hub-footer-note">
        GDPR compliant · Zero server-side processing · Open Source
      </div>
    </div>
  );
}
...
function TaskConverterUpload({ onBack }) {
  ...
  return (
    <div className="task-upload-view">
      <div className="step-container section">
        <div className="section-header">
          <div className="task-upload-header">
            <span className="task-upload-icon">✅</span>
            <h2>Converti Attività (ICS → CSV Outlook)</h2>
          </div>
        </div>
...
export default function App() {
  ...
  // Navigation state
  const [currentView, setCurrentView] = useState('hub'); // 'hub' | 'email-converter' | 'task-converter'
  ...
  const navigateTo = (view) => {
    // Reset wizard when going back to hub or switching views
    if (view === 'hub') {
      setCurrentStep(1);
      setGroups([]);
      setResolvedContacts([]);
  ...
  return (
    <div className="app">
        ...
        <div className="header-inner">
          <div className="logo-wrap">
            <div className="logo-monogram">
              <span className="char-i">i</span><span className="char-B">B</span><span className="cursor-blink">_</span>
            </div>
      ...
      {/* Wizard progress: only visible in email-converter */}
      {currentView === 'email-converter' && (
      ...
      <main className="main">
        {/* ── HUB ── */}
        {currentView === 'hub' && (
          <HubScreen onNavigate={navigateTo} t={t} />
        )}

        {/* ── TASK CONVERTER ── */}
        {currentView === 'task-converter' && (
          <TaskConverterUpload onBack={() => navigateTo('hub')} t={t} />
        )}

        {/* ── EMAIL CONVERTER WIZARD ── */}
        {currentView === 'email-converter' && (
          <>
            {currentStep === 1 && (
              <div className="step-container section">
                ...
                {groups.length > 0 && (
                  <>
                    ...
                    <div className="wizard-footer">
                      <button className="btn-outline" onClick={() => navigateTo('hub')}>← Hub</button>
            ...
            {currentStep === 4 && (
              <div className="step-container section">
                ...
                {finalContacts.length > 10 && (
                  <div className="section-desc" style={{ textAlign: 'center', marginTop: '1rem' }}>
                    {t('result.preview_more', { remaining: finalContacts.length - 10 })}
                  </div>
                )}
                <div className="wizard-footer">
